...
def ternary_classifier_plots(file='iris.csv'):
    data = pd.read_csv(file, sep=',')
    print(data.head())  # Display first few rows
    # Check for missing values
    # print(data.isnull().sum())
    # Plotting pairwise relationships
    sns.pairplot(data, hue='variety', markers=["o", "o", "o"])
    plt.savefig('plots/binary/pairplot_tri.png')  # Save the plot
